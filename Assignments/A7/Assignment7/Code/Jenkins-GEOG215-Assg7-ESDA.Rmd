...
A data frame was created to analyze the social vulnerability index per county in the contiguous US. There are `r n_obs` observations, `r n_good_obs` of which are without NAs, spread between `r min_us` and `r max_us` with a mean of `r mean_us`.
...
The histogram shows an evenly spread distribution, meaning every value in the social vulnerability index is represented in a similar number of US counties. The map shows that the spatial distribution is not nearly as even. There are visually clear clusters of high and low values.
...
The Moran's I statistic is `r us_svi_moran$estimate[1]`, with a p value of `r us_svi_moran$p.value`. This means the spatial autocorrelation for the contiguous US is somewhat dispersed and somewhat random. Because the p value is low, the results are statistically significant.
...
The LISA output reports 544 high highs and 455 low lows. clusters of the high highs and the low lows are spatially separated, with the highs in the south and lows in the north. The social vulnerability index is very high in the south and southwest and help should be directed to those areas.  
